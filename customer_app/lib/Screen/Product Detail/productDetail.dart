...
/*     _controller1.addListener(
    ...
    print('sel varient***${widget.model!.selVarient!}***$_oldSelVarient*****${widget.model!.prVarientList![widget.model!.selVarient!].id}');
    _selectedIndex.clear();
    print(
        'stocktype***${widget.model!.stockType!}***${widget.model!.availability!}');
    if (widget.model!.stockType == '0' || widget.model!.stockType == '1') {
      print('if stocktype***');
      if (widget.model!.availability == '1') {
        print('if availability');
        available = true;
        outOfStock = false;
        _oldSelVarient = widget.model!.selVarient!;
      } else {
        print('else availability');
        available = false;
        outOfStock = true;
      }
      ...
      if (widget
          ...
          '1') {
        print('if varient availability');
        available = true;
        outOfStock = false;
        _oldSelVarient = widget.model!.selVarient!;
      } else {
        print('else varient availability');
        available = false;
        outOfStock = true;
      }
    }
    ...
    print(
        'sellist***${selList.length}****${widget.model!.attributeList!.length}');
                      ...
                      choiceContainer = Padding(
                        ...
                        child: InkWell(
                          onTap: () async {
                            ...
                            print("selcted varient after****${widget.model!.selVarient}********$_oldSelVarient****${widget
                                .model!
                                .prVarientList![_oldSelVarient]
                                .id!}");
                          },
    ...
    if (isNetworkAvail) {
      try {
        ...
        if (navigatorKey.currentContext!.read<UserProvider>().userId != '') {
          parameter[USER_ID] = navigatorKey.currentContext!.read<UserProvider>().userId;
        }
        ...
        ApiBaseHelper().postAPICall(getProductApi, parameter).then(
          (getdata) {
            ...
            if (!error) {
              navigatorKey.currentContext!.read<ProductDetailProvider>().setProTotal(
                    int.parse(
                      getdata['total'],
                    ),
                  );
              ...
              if (mainlist.isNotEmpty) {
                ...
                navigatorKey.currentContext!.read<ProductDetailProvider>().setProOffset(
                      navigatorKey.currentContext!.read<ProductDetailProvider>().offset + perPage,
                    );
              }
            } else {
              if (mounted) {
        ...
        setSnackbar(getTranslated(navigatorKey.currentContext!, 'somethingMSg')!, navigatorKey.currentContext!);
        if (mounted) {
          setState(
            () {
              navigatorKey.currentContext!.read<ProductDetailProvider>().setProNotiLoading(false);
            },
          );
        }
      }
  ...
  Future<void> validatePin(String pin, bool first, bool showMsg) async {
    try {
      isNetworkAvail = await isNetworkAvailable();
      if (isNetworkAvail) {
        try {
